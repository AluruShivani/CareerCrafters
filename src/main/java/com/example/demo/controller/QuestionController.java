...
public class QuestionController {
	...
	public String saveQuestion(@ModelAttribute("question") Questions question, @RequestParam("testId") int testId, @RequestParam("vacancyId") int vacancyId,Model model) {
		...
		String url = "http://localhost:8091/CareerCrafters1/api/v1/addQuestion";
    ...
    public String getAllQuestions(Model model, HttpSession session) {
        ...
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<Questions[]> response = restTemplate.getForEntity( "http://localhost:8091/CareerCrafters1/api/v1/getQuestionsByCompanyId?companyId=" + companyId, Questions[].class);
	...
	@GetMapping("/updateQuestions/{questionId}")
	public ModelAndView updateQuestion(@PathVariable("questionId") int questionId) {
	    RestTemplate restTemplate = new RestTemplate();
	    ResponseEntity<Questions> response = restTemplate.getForEntity("http://localhost:8091/CareerCrafters1/api/v1/getQuestion/" + questionId, Questions.class);
	    Questions question = response.getBody();
	    
	    ModelAndView view = new ModelAndView("updateQuestions");
	...
	public String updateQuestion(@ModelAttribute("questionId") Questions question, Model model) {
		...
		HttpEntity<Questions> entity = new HttpEntity<>(question, headers);
		RestTemplate restTemplate = new RestTemplate();
		restTemplate.put("http://localhost:8090/api/v1/updateQuestion/" + question.getQuestionId(), entity);
